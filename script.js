...
function initNavigation() {
    ...
    window.addEventListener('scroll', () => {
        ...
        const thresholdY = navbarHeight + 20; // desktop detection line just below navbar
        ...
        // Desktop: Viewport-based detection for perfect sync with visible content
        if (isDesktop) {
            ...
            if (window.pageYOffset <= 50) {
                ...
                // Calculate which section has the most visible area in viewport
                ...
                for (const group of sectionGroups) {
                    ...
                    // Check main section
                    const mainRect = group.main.getBoundingClientRect();
                    const visibleTop = Math.max(viewportTop, mainRect.top);
                    const visibleBottom = Math.min(viewportBottom, mainRect.bottom);
                    const visibleHeight = Math.max(0, visibleBottom - visibleTop);
                    const visibleArea = visibleHeight;
                    ...
                    for (const sub of group.subsections) {
                        ...
                        const subVisibleTop = Math.max(viewportTop, subRect.top);
                        const subVisibleBottom = Math.min(viewportBottom, subRect.bottom);
                        const subVisibleHeight = Math.max(0, subVisibleBottom - subVisibleTop);
                        subsectionArea += subVisibleHeight;
                    }
                    
                    const totalArea = visibleArea + subsectionArea;
                    
                    // Prioritize sections that are actually in viewport
                    if (mainRect.top < viewportBottom && mainRect.bottom > viewportTop) {
                        if (totalArea > maxVisibleArea || (totalArea === maxVisibleArea && mainRect.top < closestSectionTop)) {
                            maxVisibleArea = totalArea;
                            closestSectionTop = mainRect.top;
                            bestSection = group.id;
                        }
                    }
                }
                
                // If no section is in viewport (gap scenario), find the closest approaching section
                if (!bestSection) {
                    ...
                    for (const group of sectionGroups) {
                        ...
                        // If section is below viewport and approaching
                        if (rect.top > viewportTop && rect.top < viewportBottom + 200) {

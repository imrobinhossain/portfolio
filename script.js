...
(function() {
    ...
    function initTheme() {
        ...
        const theme = stored || (system === 'dark' ? 'dark' : 'light'); // Default to light if no preference
        setTheme(theme);
    }
...
(function() {
    function updateExperienceCounter() {
        ...
        // Inject into the DOM
        const expElement = document.querySelector('.exp-auto');
        if (expElement) {
    ...
    // Check daily at midnight (86400000 ms = 24 hours)
    // This ensures the counter updates on the 1st of every month

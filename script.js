...
function initPreloader() {
    ...
    const originalTechFacts = [
        { text: "Cyberattacks occur every 39 seconds, putting one in three people at risk annually.", source: "Norton" },
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // PRELOADER PAUSED - Immediately hide preloader and show content
    const preloader = document.getElementById('preloader');
    if (preloader) {
        preloader.style.display = 'none';
    }
...
function initNavigation() {
    ...
    window.addEventListener('scroll', () => {
        ...
        // Special handling for Language Proficiency section
        const languageProficiencySection = document.getElementById('language-proficiency');
        let isInLanguageProficiency = false;
        
        if (languageProficiencySection) {
            const rect = languageProficiencySection.getBoundingClientRect();
            isInLanguageProficiency = (rect.top <= 100 && rect.bottom >= 100);
        }
        
        // Special handling for About Myself section
        const aboutMyselfSection = document.getElementById('about-myself');
        let isInAboutMyself = false;
        
        if (aboutMyselfSection) {
            const rect = aboutMyselfSection.getBoundingClientRect();
            isInAboutMyself = (rect.top <= 100 && rect.bottom >= 100);
        }
        ...
        // If user is in Language Proficiency section, treat it as Skills section
        if (isInLanguageProficiency) {
            current = 'skills';
        }
        
        // If user is in About Myself section, treat it as Portfolio section
        if (isInAboutMyself) {
            current = 'portfolio';
        }
...
function updateNavIndicator() {
    ...
    if (languageProficiencySection) {
        const langTop = languageProficiencySection.offsetTop;
        const langBottom = langTop + languageProficiencySection.offsetHeight;
        isInLanguageProficiency = (windowMiddle >= langTop && windowMiddle <= langBottom);
    }
    ...
    if (aboutMyselfSection) {
        const aboutTop = aboutMyselfSection.offsetTop;
        const aboutBottom = aboutTop + aboutMyselfSection.offsetHeight;
        isInAboutMyself = (windowMiddle >= aboutTop && windowMiddle <= aboutBottom);
    }
    ...
    navLinks.forEach(link => {
        ...
        if (link.getAttribute('href') === `#${currentSection}`) {
            ...
            navIndicator.style.visibility = 'visible';
        }
    });
...
function animateCount(element, target, duration = 2000, originalTargetText) {
    ...
    function updateCount(currentTime) {
        ...
        const combinedEasing = (easeOutCubic + easeOutBack) / 2;
        
        const currentValue = target * combinedEasing;
        
        if (progress < 1) {
            // For decimal numbers like 0.2+, format appropriately
            ...
            // Set final value with the correct original target text
...
function scrollCertifications(direction) {
    ...
    // Disable the CSS animation completely
...
function startAutoScroll() {
    ...
    autoScrollInterval = setInterval(() => {
        const cardWidth = track.querySelector('.certification-card').offsetWidth + 16;
        const maxScroll = track.scrollWidth - track.parentElement.offsetWidth;
        
        currentScrollPosition += 1; // Move 1px at a time for smooth scrolling
        
        if (currentScrollPosition >= maxScroll) {
            currentScrollPosition = 0; // Reset to beginning
        }
...
function initProfileImageFlip() {
    ...
    function startImageTimer() {
        ...
        // After 20 seconds, switch to informal image with flip effect
        profileFlipTimer = setTimeout(() => {
            // Add flip animation class
            profileImage.style.transform = 'rotateY(90deg)';
            ...
            // After another 20 seconds, switch back to formal image with flip effect
            profileFlipTimer = setTimeout(() => {
                // Add flip animation class
                profileImage.style.transform = 'rotateY(90deg)';
            ...
            }, 20000); // 20 seconds for informal image
        }, 20000); // 20 seconds for formal image
    }

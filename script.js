...
document.addEventListener('DOMContentLoaded', function() {
        ...
        anchor.addEventListener('click', function (e) {
            ...
            if (target) {
                ...
                if (window.innerWidth > 1024) {
                    ...
                    // Calculate position: section title top + current scroll - navbar height - consistent offset
    ...
    function scheduleNavIndicatorUpdate() {
        ...
        requestAnimationFrame(() => {
            ...
            if (window.innerWidth > 1024) updateNavIndicator();
        });
    }
...
function initNavigation() {
    ...
    window.addEventListener('scroll', () => {
        ...
        let current = '';
        const scrollThreshold = 100; // Offset from top for active section detection
...
function updateNavIndicator(targetLink) {
  try {
    ...
    if (!container) return;
    const activeLink = targetLink || container.querySelector('.nav-link.active');
    if (!activeLink) {
    ...
    // fail-safe: hide indicator
    const container = document.querySelector('#navbar .nav-container .nav-links');
    if (container) container.style.setProperty('--nav-indicator-opacity', '0');
  }
}

...
function animateProgressBar(progressFill, targetPercentage) {
    ...
    function updateProgress(currentTime) {
        ...
        if (progress < 1) {
            ...
            // Show callout immediately when progress bar stops
            const callout = progressFill.nextElementSibling;
            if (callout && callout.classList.contains('progress-callout')) {
                // Add smooth transition for callout appearance
                callout.style.transition = 'all 0.6s cubic-bezier(0.4, 0, 0.2, 1)';
                callout.style.opacity = '1';
                
                // Handle different callout positioning based on skill level

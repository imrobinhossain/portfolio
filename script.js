...
/* Automatic Experience Counter
    - Calculates tenure from 15 June 2025 to today
    - Displays as X+ format (e.g., 10+ for months, 1.0+ for years)
    - Updates on page load and every 24 hours
    - Runs identically on desktop and mobile, lag-free
    - Compares current day and joining date for accuracy */
(function() {
    function updateExperienceCounter() {
        const start = new Date(2025, 5, 15); // 15 June 2025
        const now = new Date();
        let months = (now.getFullYear() - start.getFullYear()) * 12 + (now.getMonth() - start.getMonth());
        // Add 1 month if current day is after joining day
        if (now.getDate() > start.getDate()) {
            months += 1;
        }
        months = Math.max(months, 0);
        
        // Display as X+ format (months or years)
        document.querySelectorAll('.exp-auto').forEach(el => {
            if (months < 12) {
                el.textContent = months.toString();
            } else {
                // Convert to years with 1 decimal place, rounding up
                const years = Math.ceil((months / 12) * 10) / 10;
                el.textContent = years.toFixed(1);
            }
            el.parentElement.querySelector('.exp-suffix').textContent = '+';
        });
    }
    
    // Run on page load
    document.addEventListener('DOMContentLoaded', updateExperienceCounter);
    if (document.readyState !== 'loading') {
        updateExperienceCounter();
    }
    setInterval(updateExperienceCounter, 86400000);
})();
